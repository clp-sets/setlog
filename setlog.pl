...
% (In apply, positive precondition is true because apply generates the precondition 
% by itself. I.e. apply(F,X,Y) => pfun(F) & comp({[X,X]},F,{H}))
%
preconditions(apply(F,X,_Y), (npfun(F) or comp({} with [X,X],F,{})), true) :-!.
...
sat_riseq_notemptyset(R,RR,R2,R3,F) :-                           
        (var(R),!, RR=R, sat_step(R3,R2,_,F)     
         ;
         ris_term(R),!, sat_step([RR is R,set(RR)|R3],R2,_,F)
         ;
         is_empty(R),!, RR={}, sat_step(R3,R2,_,F)
         ;
...
ris_term(T,ris(CE_Dom,LVars,Fl,P,PP)) :-  
       nonvar(T), T = ris(CE_Dom,LVars,Fl,P,PP).
...
sat_eq_cp([T1 = T2|R1],R2,c,F) :-              % cp(...) = X --> substitute X
        var(T2), 
        occur_check(T2,T1),!,
        (nonvar(T1), T1 = cp(A,_), nonvar(A), is_empty(A),!,T2={}
         ;
         nonvar(T1), T1 = cp(B,_), nonvar(B), is_empty(B),!,T2={}
         ;
         T2 = T1
        ),
        sat_step(R1,R2,_,F).
...
sunify(T1,T2,[T1 = T2]) :-                % ris = t or cp(...) = t
      nonvar(T1), 
      (ris_term(T1),! ; cp_term(T1)),!.
sunify(T1,T2,[T1 = T2]) :-                % t = ris or t = cp(...)
      nonvar(T2), 
      (ris_term(T2),! ; cp_term(T2)),!.
sunify(X,Y,C) :-                          % X = t  
      var(X),!,
...
postproc_list(K with A1,Res) :-        
        ...
        final_sat([B1 is K],_),
        postproc(A1,A2),
        (B1=={},!, Res=A2 ; postproc_list(B1,B2),Res=(A2,B2)).   
postproc_list(K with A1,Res) :-  
        is_ker(K),!, 
        postproc(K,KExt), postproc(A1,A2),
        (KExt=={},!, Res=A2 ; Res=(A2 / KExt)).   
...
unify_empty(S,C) :-   
        S = cp(A,B),
        (unify_empty(A,C) ; unify_empty(B,C)).    
...
%%% the {log} atom selection strategy (default: cfirst) 
...
%%% other aggregates, besides sets (default: off = sets only)
%other_aggrs(on).
other_aggrs(off).                  
...
%%% alternative strategies for comp-constraint rewriting (default: std)
%comp_constraint(std).             % default
%comp_constraint(sb).              % exploiting the equivalence comp(X,Y,Z) iff compsb(X,Y,Z) & sbcomp(X,Y,Z)
