...
sat_nun_([nun(S1,S2,S3)|R1],R2,c,F) :-               % nun(s1,s2,s3)
    sat_step([N in S3,N nin S1,N nin S2|R1],R2,_,F).
sat_nun_([nun(S1,_S2,S3)|R1],R2,c,F) :-              %
    sat_step([N in S1,N nin S3|R1],R2,_,F).
...
sat_ninv_([ninv(R,S)|R1],R2,c,F) :-                % ninv(R,S)
